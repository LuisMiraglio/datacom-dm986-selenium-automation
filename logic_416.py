...
class ConfiguradorModem416:
    """
    Lógica específica Datacom DM986-416 AX30 (SIN UI).
    Se ejecuta desde main.py que provee 'ui' con métodos:
      ...
      ui.get_extra_wifi_config() -> dict:
          {
            "enabled": bool,
            # valores "value" reales del HTML del 416
            "chanwid_5": "0|1|2|3",        # 20/40/80/160
            "chan_5": "0|36|40|...",       # 0=DFS
            "chanwid_24": "0|1|3",         # 20/40/20-40
            "chan_24": "0|5|6|..."         # 0=Auto
          }

    Si tu main.py hoy devuelve textos tipo "80MHz", "DFS", etc,
    avisame y lo adapto en 2 minutos (pero lo ideal es que main devuelva values).
    """
    ...
    def run(self):
        try:
            ...
            self._status("✅ Configuración DM986-416 completada.")

        except Exception as e:
            self._msgbox_error("Error durante la configuración (416)", str(e))
            traceback.print_exc()
    ...
    def _normalize_extras(self, extras: dict) -> dict:
        """
        Asegura que existan defaults correctos para 416:
          - 5GHz: 80MHz -> value "2", DFS -> "0"
          - 2.4GHz: 20MHz -> "0", Auto -> "0"
        """
        ...
        # Defaults firmwares 416 (según tu HTML)
        out = {
            "enabled": enabled,
    ...
    def configurar_modem(self, creds: dict, extra: dict):
        ...
        self._status("Configurando WiFi 5GHz...")
        wlan_link = nav_menu.find_element(By.XPATH, ".//a[@rel='3' and normalize-space()='WLAN']")
        self._click_safe(wlan_link)

        self._switch_to_content_iframe(timeout=15)

        ssid_input = WebDriverWait(d, 10).until(EC.presence_of_element_located((By.NAME, "ssid")))
        ssid_input.clear()
        ssid_input.send_keys(ssid_name)
        time.sleep(0.6)

        # Extras 5GHz (si enabled, trae valores seleccionados; si no, defaults)
        self._status("Aplicando Channel Width / Channel Number (5GHz)...")
        self._select_value_by_name("chanwid", extra["chanwid_5"], timeout=10)
        self._select_value_by_id("chan_select", extra["chan_5"], timeout=10)
        time.sleep(0.6)
        ...
        psk_5 = WebDriverWait(d, 10).until(EC.presence_of_element_located((By.ID, "wpapsk")))
        psk_5.clear()
        psk_5.send_keys(wpa_password)
        time.sleep(0.6)

        apply_sec5 = WebDriverWait(d, 10).until(
            EC.element_to_be_clickable((By.XPATH, "//input[@type='submit' and @name='save' and @value='Apply Changes']"))
        )
        ...
        self._select_value_by_id("chan_select", extra["chan_24"], timeout=10)
        time.sleep(0.6)
        ...
        psk_24 = WebDriverWait(d, 10).until(EC.presence_of_element_located((By.ID, "wpapsk")))
        psk_24.clear()
        psk_24.send_keys(wpa_password)
        time.sleep(0.6)

        apply_sec24 = WebDriverWait(d, 10).until(
            EC.element_to_be_clickable((By.XPATH, "//input[@type='submit' and @name='save' and @value='Apply Changes']"))
        )
